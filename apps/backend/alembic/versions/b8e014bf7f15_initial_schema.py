...
Revision ID: b8e014bf7f15
Revises: 
Create Date: 2026-09-19 19:06:00.466044
...
def upgrade() -> None:
    ...
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('matter_id', sa.Uuid(), nullable=True, comment='Matter boundary; null is reserved for curated global sources'),
    sa.Column('source_type', sa.String(length=64), nullable=False, comment='client_record, statute, rule, form, judgment, synthetic_fixture'),
    sa.Column('canonical_title', sa.String(length=512), nullable=False),
    sa.Column('authority_level', sa.String(length=64), nullable=False, comment='matter_evidence, official_primary, curated_primary, discovery_only'),
    ...
    sa.Column('confidentiality', sa.String(length=64), server_default=sa.text("'private'"), nullable=False),
    ...
    op.create_index(op.f('ix_document_versions_draft_id'), 'document_versions', ['draft_id'], unique=False)
    ...
    sa.Column('extraction_method', sa.String(length=64), nullable=True, comment='pymupdf, pdfplumber, textract, etc.'),
    sa.Column('extraction_version', sa.String(length=32), nullable=True),
    sa.Column('extraction_status', sa.String(length=64), server_default=sa.text("'pending'"), nullable=False, comment='pending, processing, completed, failed'),
    sa.Column('page_count', sa.Integer(), nullable=True),
    sa.Column('reviewed_by_human', sa.Boolean(), server_default=sa.text('false'), nullable=False),
    ...
    op.create_index(op.f('ix_source_versions_file_sha256'), 'source_versions', ['file_sha256'], unique=False)
    ...
    sa.ForeignKeyConstraint(['document_id'], ['drafts.id'], ),
    sa.ForeignKeyConstraint(['matter_id'], ['matters.id'], ondelete='CASCADE'),
    ...
    sa.Column('from_offset', sa.Integer(), nullable=False),
    sa.Column('to_offset', sa.Integer(), nullable=False),
    ...
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('document_version_id', 'block_index', 'from_offset', 'to_offset', 'claim_sha256', name='uq_claim_version_location_hash')
    )
    op.create_index(op.f('ix_claims_claim_sha256'), 'claims', ['claim_sha256'], unique=False)
    op.create_index(op.f('ix_claims_document_version_id'), 'claims', ['document_version_id'], unique=False)
    op.create_table('document_commands',
    sa.Column('user_id', sa.Uuid(), nullable=False),
    sa.Column('draft_id', sa.Uuid(), nullable=False),
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('user_id', 'document_version_id', 'idempotency_key', name='uq_draft_export_key')
    )
    op.create_index(op.f('ix_draft_exports_document_version_id'), 'draft_exports', ['document_version_id'], unique=False)
    ...
    op.create_index(op.f('ix_source_pages_source_version_id'), 'source_pages', ['source_version_id'], unique=False)
    ...
    op.create_index(op.f('ix_findings_document_version_id'), 'findings', ['document_version_id'], unique=False)
    ...
    sa.Column('heading_path', sa.JSON().with_variant(postgresql.ARRAY(sa.String()), 'postgresql'), nullable=True),
    ...
    sa.Column('created_by', sa.String(length=128), nullable=False, comment='writer_agent, user, evaluator, etc.'),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    ...
    op.create_index(op.f('ix_evidence_spans_chunk_id'), 'evidence_spans', ['chunk_id'], unique=False)
    op.create_index(op.f('ix_evidence_spans_page_id'), 'evidence_spans', ['page_id'], unique=False)
    op.create_index(op.f('ix_evidence_spans_source_version_id'), 'evidence_spans', ['source_version_id'], unique=False)
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('user_id', 'finding_id', 'idempotency_key', name='uq_finding_resolution_key')
    )
    op.create_index(op.f('ix_finding_resolutions_finding_id'), 'finding_resolutions', ['finding_id'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_claims_document_version_id'), table_name='claims')
    op.drop_index(op.f('ix_claims_claim_sha256'), table_name='claims')

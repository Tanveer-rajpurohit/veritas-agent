  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-5 pt-20 sm:px-8">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <Reveal className="flex min-h-[22rem] flex-col rounded-[1.75rem] bg-[#f4f4f6] p-7 sm:p-9">
          ...
          <div className="flex flex-1 flex-col items-center justify-center pb-6">
            <div className="flex w-full max-w-xs flex-col items-center gap-2 rounded-2xl border-2 border-dashed border-line-strong bg-white/60 px-4 py-8">
              <span className="text-[13px] font-medium text-ink">Drop records here</span>
              <span className="rounded-lg bg-white px-3 py-1.5 text-xs font-medium text-ink shadow-sm">Browse files</span>
            </div>
          </div>
        ...
        <Reveal delay={0.08} className="flex min-h-[22rem] flex-col rounded-[1.75rem] bg-[#f4f4f6] p-7 sm:p-9">
          ...
          <div className="grid flex-1 grid-cols-2 content-center gap-3 pb-6">
            ...
            <div className="rounded-xl bg-white p-3 shadow-sm">
              ...
              <span className="rounded-full bg-ok-wash px-2 py-0.5 text-[10px] font-semibold text-ok-ink">Linked</span>
            </div>
          </div>
      ...
      <div className="grid gap-5 pt-5 sm:grid-cols-3">
        {SMALL.map((item, index) => (
          <Reveal key={item.title} delay={Math.min(index * 0.06, 0.12)} className="flex flex-col rounded-[1.75rem] bg-muted p-7">
            <span className="mb-4 flex h-9 w-9 items-center justify-center rounded-full bg-white font-display text-base text-ink-accent italic shadow-sm">
              {index + 1}
            </span>
